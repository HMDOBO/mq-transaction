...
public class ScanPayNotifyController {
    ...
    public void notify(@PathVariable("payWayCode") String  payWayCode , HttpServletRequest httpServletRequest , HttpServletResponse httpServletResponse) throws Exception {
        ...
        // messageId自定义消息id，messageBody消息内容（是否成功，交易订单号，交易时间），NotifyDestinationNameEnum.BANK_NOTIFY.name()队列名称
        ...
        // 存储并发送消息到MQ消息队列
        /*
         * 这里并没有用到预发送消息，然后等待确认之后再进行消息发送
         * 因为银行在确认用户付款后，调用支付网关，在一段时间内若支付网关没有返回成功消息，会再次调用支付网关
         */
        // 分布式事务解决方案：可靠消息最终一致性--独立消息服务
