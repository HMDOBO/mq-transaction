...
//		listMerchantKey.add(new MerchantKey("ca6577dff6d647ac882dfb405ceda21e", "1b8da6c9b7544856955fcff6bf920f84"));//商户5 hpt
	...
	private static void testGateWayPay( String payKey , String paySecret){
		...
		for (int i = 0; i < 11; i++) { //每条线程生成数
			...
			// ==========step5、模拟银行扣款成功，通知回调支付网关==========
			// 支付网关通过可靠消息服务通知各个服务系统做出对应的处理
			//模拟构建银行扣款成功结果通知
			Map<String , Object> notifyMap = getNotifyRequestMap(merchantOrderNo);
			String notifyResultStr = SimpleHttpUtils.httpGet("http://192.168.25.204:8082/roncoo-pay-web-gateway/scanPayNotify/notify/TEST_PAY_HTTP_CLIENT", notifyMap);
	...
	private static Map<String , Object> getInitRequestMap(String merchantOrderNo , String payKey , String paySecret){
        ...
        paramMap.put("productName","SHANGPINGMINGCHENG");// 商品名称
        paramMap.put("orderIp","127.0.0.1");
        paramMap.put("orderPeriod","30"); // 订单有效期
        paramMap.put("returnUrl","http://192.168.25.204:8083/roncoo-pay-web-sample-shop/roncooPayNotify/notify"); // 页面通知返回url
        paramMap.put("notifyUrl","http://192.168.25.204:8083/roncoo-pay-web-sample-shop/roncooPayNotify/notify");// 后台消息通知Url
        paramMap.put("remark"," ZHIFUBEIZHU"); // 支付备注

        ////////////扩展字段,选填,原值返回///////////
        String field1 = "field1"; // 扩展字段1
        paramMap.put("field1",field1);
        String field2 = "field2"; // 扩展字段2
        paramMap.put("field2",field2);
        String field3 = "field3"; // 扩展字段3
        paramMap.put("field3",field3);
        String field4 = "field4"; // 扩展字段4
        paramMap.put("field4",field4);
        String field5 = "field5"; // 扩展字段5
